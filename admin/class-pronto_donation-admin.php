					...
					<tbody>
						<tr>
							...
							<td>
								...
								<option value="not_included">Select Salesforce GUI</option>
								<?php 
						...
						<tr>
							...
							<?php 
								...
								$editor_id = 'campagin_description';
						...
						<tr>
							...
							<td>
								<select name="donation_type" id="donation_type">
									<option value="single" <?php if( !empty( $campaign_info['donation_type'] ) && esc_attr($campaign_info['donation_type']) == 'single' ) echo "selected='selected'"; ?> >One-off</option>
									<option value="recurring" <?php if( !empty( $campaign_info['donation_type'] ) && esc_attr($campaign_info['donation_type']) == 'recurring' ) echo "selected='selected'"; ?> >Monthly</option>
		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				...
				$('#add_amount_btn').click(function(e){
					...
					if(parseData < 2) {
						$(this).after("<small id='amount-err' style='color:red;'>  Minimum donation amount per transaction is 2 "+ currecyCode +". Thank you.</small>")
						$('#amount_level').val('');
		...
		if( $get_post_campaign == 'campaign' ) {
			...
			if( isset( $_POST['publish'] ) || isset( $_POST['save'] ) ) {
				...
				if( isset( $_POST['campagin_description'] ) ) {
					$result = $wpdb->query("UPDATE {$wpdb->prefix}posts SET post_content='".$_POST['campagin_description']."' WHERE ID=".$post_id."");
				}
